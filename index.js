...
export default class RNSketchCanvas extends React.Component {
  ...
  async save() {
    if (this.props.savePreference) {
      ...
      return this._sketchCanvas.save(p.imageType, p.transparent, p.folder ? p.folder : '', p.filename, p.includeImage !== false, p.includeText !== false, p.cropToImageSize || false)
    } else {
      const date = new Date()
      return this._sketchCanvas.save('png', false, '',
